...
class HousingComScraper extends ScraperAgent {
    constructor() {
        super('housing_com', {
    ...
    async scrape({ city, locality, propertyType, bedrooms }) {
        ...
        if (process.env.SCRAPER_USE_MOCK !== 'false') {
            const listings = getMockListingsForCity(city, 'housing_com');
        ...
        try {
            ...
            if (!cityConfig || !cityConfig.housing_com) {
                this.logAction('scrape_skip', { city, reason: 'no_url_config' });
                return [];
            }

            let url = `${this.baseUrl}${cityConfig.housing_com}`;
            if (locality) url += `?q=${encodeURIComponent(locality)}`;
    ...
    parseListings(html, city) {
        ...
        $('[class*="listing-card"], [class*="property-card"]').each((_, el) => {
            try {
                const title = $(el).find('[class*="title"], h2, h3').first().text().trim();
                const priceText = $(el).find('[class*="price"]').first().text().trim();
                const areaText = $(el).find('[class*="area"], [class*="size"]').first().text().trim();
                const localityText = $(el).find('[class*="address"], [class*="locality"]').first().text().trim();
                ...
                listings.push({
                    source: 'housing_com',
                    sourceListingId: `hc_${Buffer.from(link || title || String(Date.now())).toString('base64').slice(0, 20)}`,
...
module.exports = HousingComScraper;
